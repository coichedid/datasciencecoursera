...
## It plots lines by colors provided 
## Then if required (showLegend == TRUE) plots a legend with each line
plot.study <- function(data, xvar, yvars ,xlab = "", ylab = "", showLegend = FALSE) {
    with(data,{
...
savetofile <- TRUE ## Set device to PNG
...
par(mfrow = c(2,2)) ## Set canvas to 2 columns and 2 rows

## Plot Global Active Power By DateTime
plot.study(data,xvar = "DateTime", 
           yvars = data.frame(var = c("Global_active_power"),
                              color = c("black"), stringsAsFactors = FALSE),
           ylab = "Global Active Power",
           showLegend = FALSE)

## Plot Voltage By DateTime
plot.study(data,xvar = "DateTime", 
           yvars = data.frame(var = c("Voltage"),
                              color = c("black"), stringsAsFactors = FALSE),
           ylab = "Voltage",
           xlab = "datetime",
           showLegend = FALSE)

## Plot Energy sub metering By DateTime
plot.study(data,xvar = "DateTime", 
...
plot.study(data,xvar = "DateTime", 
           yvars = data.frame(var = c("Global_reactive_power"),
                              color = c("black"), stringsAsFactors = FALSE),
